...
const Fruit = ({name, description, background}) => {
  ...
  const capitalizedBackground = `fruit${background.charAt(0).toUpperCase() + background.slice(1).toLowerCase()}`;
  ...
  const fruitImage = fruitImages[name.toLowerCase()] || watermelon;

  return (
    <div className={`bg-${capitalizedBackground} flex flex-col rounded-lg p-5 gap-2`}>
